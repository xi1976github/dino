...
public class FileWidget : Box {
    ...
    private async Widget? get_image_widget(FileTransfer file_transfer) {
        ...
        Button open_button1 = builder.get_object("open_button1") as Button; //xi
        Util.force_css(open_button, "*:hover { background-color: rgba(255,255,255,0.3); border-color: transparent; }");
        Util.force_css(open_button1, "*:hover { background-color: rgba(255,255,255,0.3); border-color: transparent; }"); //xi
        ...
        open_button1.clicked.connect(() => {
            Gdk.Display disp = Gdk.Display.get_default();            
            Gtk.Clipboard cb = Gtk.Clipboard.get_for_display(disp,Gdk.SELECTION_CLIPBOARD);
            cb.set_image(pixbuf2);
            cb.store();
        });
